...
const ListadoTratamiento = () => {
  ...
  // 🆕 NUEVO: Función para eliminar tratamiento
  const eliminarTratamiento = async () => {
    ...
    try {
      console.log('🗑️ Eliminando tratamiento:', modalEliminar.tratamiento.id_tratamiento);
      ...
      if (response.ok) {
        showAlert(`✅ Tratamiento "${modalEliminar.tratamiento.nombre}" eliminado correctamente`);
        cargarTratamientoList(); // Recargar lista
        cerrarModal();
        ...
        showAlert(`❌ Error al eliminar tratamiento (${response.status})`, "error");
      }
    } catch (error) {
  ...
  const getTipoEnfermedadColor = (tipo) => {
    ...
    } else if (tipoLower.includes('neumonia') || tipoLower.includes('respiratorio')) {
      return "bg-red-500 text-red-900";
    } else if (tipoLower.includes('deshidratacion') || tipoLower.includes('deshidratación')) {
      return "bg-blue-500 text-blue-900";
    } else if (tipoLower.includes('infeccion') || tipoLower.includes('infección')) {
    ...
    } else if (tipoLower.includes('parasito') || tipoLower.includes('parásito')) {
      return "bg-indigo-500 text-indigo-900";
    } else if (tipoLower.includes('vitamin') || tipoLower.includes('suplemento')) {
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleFiltroChange("tipo_enfermedad", e.target.value)}
          ...
          <table className='w-full text-left table-auto min-w-max bg-gradient-to-b from-gray-800 via-gray-700 to-gray-800 border-separate border-spacing-0 rounded-lg shadow-2xl'>
            <thead className='bg-slate-900'>
              <tr>
                ...
                <th className='px-4 py-2 border-b border-slate-600 bg-slate-700'>
                  <p className='text-sm font-medium leading-none'>Tipo Enfermedad</p>
                </th>
                <th className='px-4 py-2 border-b border-slate-600 bg-slate-700'>
        ...
        {!loading && tratamientos.length > 0 && (
          <div className='mt-4 p-4 bg-slate-700 rounded-lg'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-400'>
              <div>
                <p className='mb-1'>
                  <strong>💊 Tratamientos:</strong> Gestión completa de medicamentos y terapias
                </p>
                <p>
                  <strong>🔍 Filtros:</strong> Busca por tipo de enfermedad con texto libre y turno
                </p>
              </div>
              <div>
                <p className='mb-1'>
                  <strong>📊 Estadísticas:</strong> Resumen visual de los tipos más comunes
                </p>
                <p>
                  <strong>✨ Recientes:</strong> Los primeros 3 tratamientos están marcados como recientes
                </p>
              </div>
            </div>
          </div>
