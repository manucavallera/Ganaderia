...
// ⬅️ ELIMINADO: import SeleccionarPadre from './Select-Padre';
...
const FormularioMadre = ({ setStep }) => {
  ...
  // ⬅️ ELIMINADO: const [idPadres, setIdPadres] = useState([]);
  ...
  // ⬅️ ELIMINADO: const handleObetenerPadre = (id) => { ... }
  ...
  const handleNext = async (data) => {
    ...
    if (resMadreCreada?.status == 201) {
      setMadreAlert({
        status: true,
        message: "SE HA REGISTRADO CORRECTAMENTE LA MADRE",
        estado: true,
      });
      reset();
      // ⬅️ ELIMINADO: setIdPadres([]);
    } else {
      setMadreAlert({
        status: true,
        message: "ERROR 401, SESSION CADUCADA",
        estado: false,
      });
      reset();
      // ⬅️ ELIMINADO: setIdPadres([]);
        ...
        <form onSubmit={handleSubmit(handleNext)}>
          ...
          <div className='mb-4'>
            ...
            <input
              type='number'
              {...register("rp_madre", {
                required: "El RP Madre es obligatorio",
              })}
          ...
          {/* ⬅️ ELIMINADO: Sección completa de seleccionar padres */}
