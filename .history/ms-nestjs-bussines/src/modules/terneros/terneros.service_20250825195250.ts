...
// Importar el DTO de calostrado del controller
...
export class TernerosService {
  ...
  async create(createTerneroDto: CreateTerneroDto): Promise<TerneroEntity> {
    try {
      ...
      // Preparar datos para crear el ternero
      const datosTernero: Partial<TerneroEntity> = {
        ...createTerneroDto,
      ...
      if (createTerneroDto.fecha_hora_calostrado) {
        datosTernero.fecha_hora_calostrado = new Date(
          createTerneroDto.fecha_hora_calostrado,
        );
      }

      // Crear el ternero
      const nuevoTernero: TerneroEntity =
        this.terneroRepository.create(datosTernero);

      const terneroGuardado: TerneroEntity =
        await this.terneroRepository.save(nuevoTernero);
