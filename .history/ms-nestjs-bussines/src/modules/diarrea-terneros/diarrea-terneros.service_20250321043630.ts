...
export class DiarreaTernerosService {
  ...
  async create(createDiarreaTerneroDto: CreateDiarreaTerneroDto): Promise<DiarreaTerneroEntity> {
      ...
      if (!ternero) {
          throw new Error(`No se encontró el ternero con el ID ${createDiarreaTerneroDto.id_ternero}`);
      }
      ...
      async findAll(): Promise<DiarreaTerneroEntity[]> {
        try {
          ...
          throw new HttpException(`Error al obtener todas las diarreas: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
      }
      ...
      async findOne(id: number): Promise<DiarreaTerneroEntity> {
        try {
          ...
          const diarreaTernero = await this.diarreaRepository.findOne({where:{id_diarrea_ternero:id},
          relations:['ternero']
          });
          ...
          throw new HttpException(`Error al obtener la diarrea con el ID ${id}: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
      }
    
      async update(id: number, updateDiarreaTerneroDto: UpdateDiarreaTerneroDto): Promise<DiarreaTerneroEntity> {
        try {
          ...
          if (!diarreaTernero) {
            throw new HttpException('Diarrea Ternero no encontrada', HttpStatus.NOT_FOUND);
          }
      ...
      async remove(id: number): Promise<{ message: string }> {
        try {
          ...
          const diarrea = await this.diarreaRepository.findOne({where:{
          ...
          throw new HttpException(`Error al eliminar la diarrea con ID ${id}: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
      }
}
