...
export class EventosService {
  ...
  // ⬅️ MÉTODO CORREGIDO: Cada evento con sus propios animales únicos
  async createMultiple(
    ...
    try {
      ...
      for (const eventoDto of createEventosData.eventos) {
        // Buscar terneros específicos para este evento
        const terneros =
        ...
        // Buscar madres específicas para este evento
        const madres =
        ...
        // Validaciones opcionales (comentadas para mayor flexibilidad)
        // if (eventoDto.id_ternero?.length > 0 && terneros.length === 0) {
        //   throw new HttpException(
        //     `No se encontraron terneros con los IDs: ${eventoDto.id_ternero.join(', ')}`,
        //     HttpStatus.BAD_REQUEST,
        //   );
        // }

        // if (eventoDto.id_madre?.length > 0 && madres.length === 0) {
        //   throw new HttpException(
        //     `No se encontraron madres con los IDs: ${eventoDto.id_madre.join(', ')}`,
        //     HttpStatus.BAD_REQUEST,
        //   );
        // }

        // Crear el evento con sus animales específicos
        const eventoCreado = this.eventoRepository.create({
          fecha_evento: eventoDto.fecha_evento,
          observacion: eventoDto.observacion,
          terneros, // ⬅️ Terneros únicos para este evento
          madres, // ⬅️ Madres únicas para este evento
        });
